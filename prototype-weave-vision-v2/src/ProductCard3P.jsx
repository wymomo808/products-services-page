...
import { OrgStatusBadge } from "./TrustBadges.jsx";
  ...
  return (
    ...
    >
      {showCornerBadges ? (
        ...
        >
          {showDeployedBadge ? <OrgStatusBadge status="Deployed" /> : null}
          {product.isAutodesk ? <AutodeskBadge /> : null}
        </Box>
      ) : null}
      {/* body */}
      <Box sx={{ display: "flex", flexDirection: "column", gap: "28px", flex: 1, p: "24px" }}>
        {/* slot-01: lockup */}
        <Box sx={{ display: "flex", flexDirection: "column", gap: "8px", pr: showCornerBadges ? "92px" : 0 }}>
        ...
        {/* slot-02: description + deployed badge for solutions */}
        <Box sx={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {!isProduct && (product.worksWith || []).length ? (
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          {!isProduct && typeof product.assignedUsers === "number" && product.assignedUsers > 0 ? (
            <Typography sx={{ ...VIS_D.typography.smallprint, color: VIS_D.colors.textLight, mt: "4px" }}>
              {product.assignedUsers} users assigned
            </Typography>
          ) : null}
