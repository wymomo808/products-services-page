...
import { CaretDownS, CaretUpS, InfoS } from "@weave-mui/icons-weave";
import AssignmentRowActions, { OutlinedPermissionGroupIcon } from "./AssignmentRowActions.jsx";
                  ...
                  {visibleGroups.map((group) => {
                    ...
                    return renderSelectRow({
                      ...
                      leading: (
                        ...
                        >
                          <OutlinedPermissionGroupIcon sx={{ width: 18, height: 18 }} />
                        </Box>
                      ),
                      title: group.name,
...
function AssignedUsersTable({ users, onAction }) {
  ...
  return (
    ...
    >
      ...
      <Box
        sx={{
          display: "grid",
          gridTemplateColumns: "minmax(0, 1fr) 140px minmax(260px, 1.2fr)",
          alignItems: "center",
          gap: "16px",
          px: "16px",
          py: "12px",
      ...
      >
        <Box sx={{ display: "inline-flex", alignItems: "center", gap: "6px" }}>
          <Typography sx={columnHeaderSx}>Name</Typography>
          <CaretUpS sx={{ width: 16, height: 16, color: VIS_D.colors.ink }} />
        </Box>
        <Box sx={{ display: "inline-flex", alignItems: "center", gap: "6px" }}>
          <Typography sx={columnHeaderSx}>Assigned from</Typography>
          <InfoS sx={{ width: 16, height: 16, color: VIS_D.colors.textLight }} />
        </Box>
      </Box>
      ...
      {users.map((user, index) => (
        <Box
          ...
          sx={{
            ...
            px: "16px",
            py: "14px",
            borderBottom: index < users.length - 1 ? `1px solid ${VIS_D.colors.divider}` : "none",
            bgcolor: VIS_D.colors.background,
          }}
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: "12px", minWidth: 0 }}>
            <Avatar
              sx={{
                width: 32,
                height: 32,
                bgcolor: user.avatarColor ?? VIS_D.colors.panel,
                fontSize: "12px",
                fontWeight: 700,
                flexShrink: 0,
              }}
            >
              {user.initials ?? user.displayName?.slice(0, 2)}
            </Avatar>
            <Typography
              sx={{
                ...VIS_D.typography.label16Semi,
          ...
          <Typography sx={{ ...VIS_D.typography.bodyMedium, fontFamily: FONT, color: VIS_D.colors.textPrimary }}>
            {user.assignedFrom ?? "Pool"}
          </Typography>

          <AssignmentRowActions
            label={user.displayName ?? user.email}
            onUnassign={() => onAction(`Unassign ${user.displayName ?? user.email}`)}
            onView={() => onAction(`View user ${user.displayName ?? user.email}`)}
          />
        </Box>
      ))}
    </Box>
    ...
    <>
      ...
      <Box sx={detailCardSx}>
        ...
        <Button
          ...
          sx={{ ...outlineBtnSx, alignSelf: "flex-start", mt: "16px" }}
        >
          View subscription
        </Button>
      </Box>
      ...
      <Box sx={detailCardSx}>
        <Box sx={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <AssignedUsersTable users={detail.assignment.assignedUsers} onAction={onAction} />
        </Box>
      </Box>
    </Box>
    </>
